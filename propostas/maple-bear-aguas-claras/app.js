...
(() => {
  ...
  /* ---------- 11. LAYOUT INTERATIVO (Águas Claras) ---------- */
  // Clicar numa caixa do layout abre a foto correspondente no lightbox.
  // Reusa a função openLightbox via simulação de click no .toy-zoom respectivo.
  document.querySelectorAll('.layout-toy').forEach((g) => {
    const idx = +g.dataset.idx;
    const trigger = () => {
      const zoom = document.querySelector('.toy-zoom[data-open="' + idx + '"]');
      if (zoom) zoom.click();
    };
